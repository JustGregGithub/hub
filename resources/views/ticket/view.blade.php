        ...
        <div class="flex flex-wrap lg:flex-nowrap mt-4 gap-4">
            ...
            <div class="w-full lg:w-3/4">
                            ...
                            <div class="bg-yellow-50 border border-yellow-100 rounded-xl p-4 mt-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="flex justify-between w-full">
                                        <div>
                                            <p class="break-all">{{ $reply->user->displayName()  }} @if ($reply->user->discriminator != 0) #{{ $reply->user->discriminator }} @endif</p>
                                            <p class="text-xs text-gray-400 break-all">{{ $reply->user_id  }}</p>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-xs text-gray-400 break-all">Sent: {{ $reply->created_at->diffForHumans() }}</p>
                                            @if($reply->created_at != $reply->updated_at)
                                                <p class="text-xs text-gray-400 break-all">Updated: {{ $reply->updated_at->diffForHumans() }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                <form method="POST" action="{{ route('tickets.reply.post', $ticket->slug)  }}" class="mt-4">
                    ...
                    <div class="flex gap-2">
                        @can('can-support', $ticket)
                            <select name="type" class="border border-gray-300 rounded-md text-sm mt-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option value="0">Reply</option>
                                <option value="1">Note</option>
                            </select>
                        @endcan
                        <input type="submit" class="bg-purple-300 hover:bg-purple-500 rounded-md p-2 text-purple-500 hover:text-white mt-2 w-full cursor-pointer transition" value="Submit Reply">
                    </div>
                </form>
            </div>
            ...
            <div class="w-full lg:w-1/4">
                ...
                <div class="bg-white dark:bg-slate-600 dark:text-gray-300 w-full rounded-xl p-4 text-sm mt-2">
                    ...
                    <small class="text-{{ \App\Models\Ticket::statusColor($ticket->status) }}">{{ \App\Models\Ticket::status($ticket->status) }}</small>
                    ...
                    <small>{{ $ticket->getAssigneeName() }} <span class="text-gray-400">({{ $ticket->assigned_person }})</span></small>
                </div>
                    ...
                    <div class="bg-white dark:bg-slate-600 w-full rounded-xl p-4 text-sm mt-2">
                        ...
                        @if($allowed_users->count() > 0)
                            <form method="POST" action="{{ route('tickets.user.delete', $ticket) }}" class="flex gap-2">
                                ...
                                <input type="submit" value="Remove" class="bg-red-500 hover:bg-red-400 transition rounded-md px-4 py-2 text-white w-fit mt-2 cursor-pointer">
                            </form>
                    ...
                    <div class="bg-white dark:bg-slate-600 w-full rounded-xl p-4 text-sm mt-2">
                        ...
                        <form method="POST" action="{{ route('tickets.update.post', $ticket->slug) }}">
                            ...
                            @if ($ticket->assigned_person)
                                ...
                                <input type="submit" value="Quick Un-Assign" class="bg-red-500 hover:bg-red-400 transition rounded-md px-4 py-2 text-white w-full mt-2 cursor-pointer">
                            @else
                                <input type="hidden" name="assigned_person" value="{{ Auth::user()->id }}" />
                                <input type="submit" value="Assign to Me" class="bg-green-500 hover:bg-green-400 transition rounded-md px-4 py-2 text-white w-full mt-2 cursor-pointer">
                            @endif
                        </form>
                    </div>
                @endcan
