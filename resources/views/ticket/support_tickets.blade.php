    ...
    <div class="p-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-100 w-full">
                <div class="mt-10">
                    <h1 class="text-xl font-bold">Ticket Support / {{ $category->name }}</h1>
                    <div class="bg-white rounded-xl p-4 mt-4 flex flex-col gap-4">
                        <span class="font-bold">Category Statistics</span>
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach(\App\Models\Ticket::STATUSES as $key=>$value)
                                <div class="bg-{{ \App\Models\Ticket::statusColor($value) }} text-center p-6 rounded-md">
                        ...
                        <div class="bg-gray-100 rounded-md p-4">
                            <span class="font-bold">Assigned Tickets ({{\App\Models\Ticket::where('assigned_person', Request::user()->id, true)->where('category_id', $category->id)->get()->count()}})</span>
                                    ...
                                    @foreach($category->tickets as $ticket)
                                        @if($ticket->assigned_person == Request::user()->id)
                                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                ...
                                                <td class="px-6 py-4 text-{{ App\Models\Ticket::statusColor($ticket->status) }}">
                        ...
                        @foreach (\App\Models\Ticket::STATUSES as $key=>$value)
                            <div class="bg-gray-100 rounded-md p-4">
                                <span class="font-bold">{{ $key }} Tickets ({{ $category->statusCount($value) }})</span>
