                ...
                @foreach ($application_sections as $application_section)
                    @if($application_section->categories->where('is_open', true)->isNotEmpty())
                        <div class="mt-5 rounded-xl p-4"
                             style="background: linear-gradient(138deg, {{ $application_section->colour_left }} 27%, {{ $application_section->colour_right }} 100%);">
                                ...
                                @foreach ($application_section->categories as $category)
                                    @if($category->is_open && !$category->restrict)
                                        <div class="border rounded-xl p-4 text-center bg-gray-50 dark:bg-slate-600 dark:text-gray-300 dark:border-gray-400 h-fit">
                                            ...
                                            <div class="mt-6">
                                                <a href="@if(\App\Models\Hub\Application::where('application_category_id', $category->id)->where('user_id', Request::user()->id)->where('status', \App\Models\Hub\Application::STATUSES['Under Review'])->exists()) #" @else
                                                    {{ route('applications.apply', $category->id) }}"
                                                @endif class="bg-purple-200 px-4 py-2 rounded-md
                                                text-purple-600 @if(\App\Models\Hub\Application::where('application_category_id', $category->id)->where('user_id', Request::user()->id)->where('status', \App\Models\Hub\Application::STATUSES['Under Review'])->exists())
                                                    cursor-not-allowed
                                                @endif">
                                                Apply Now
                                                </a>
