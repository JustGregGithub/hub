                            ...
                            <div class="mt-2">
                                ...
                                @switch($question->type)
                                    ...
                                    @case(\App\Models\Hub\ApplicationQuestion::OPTION_TYPES['Select'])
                                        ...
                                        @break
                                    @case(\App\Models\Hub\ApplicationQuestion::OPTION_TYPES['Radio'])
                                            ...
                                            @foreach ($question->options as $key => $value)
                                                <div>
                                                    <input type="radio"
                                                           ...
                                                           name="questions[{{ $question->id }}]"
                                                           value="{{ $key }}"
                                                           class="dark:bg-gray-700 dark:checked:bg-blue-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 "
                                                            {{ old('questions.'.$question->id) == $value ? 'checked' : '' }}
                                                    >
                                                    <label for="radio-{{ $value }}"
                                                           class="dark:text-gray-400">{{ $key }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                        @break
