                ...
                <div>
                    <span class="relative z-0 inline-flex rounded-md shadow-sm">
                        <span>
                            ...
                            @else
                                <button wire:click="previousPage('{{ $paginator->getPageName() }}')" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" rel="prev" class="relative inline-flex items-center px-2 py-2 text-sm font-medium relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium shadow-sm transition duration-150 ease-in-out sm:text-sm sm:leading-5 focus:border-purple-300 focus:ring focus:ring-purple-200 focus:ring-opacity-50 bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-600 rounded-tl-md rounded-bl-md" aria-label="{{ __('pagination.previous') }}">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                    </svg>
                                </button>
                                        ...
                                        <span wire:key="paginator-{{ $paginator->getPageName() }}-{{ $this->numberOfPaginatorsRendered[$paginator->getPageName()] }}-page{{ $page }}">
                                            @if ($page == $paginator->currentPage())
                                                <span aria-current="page">
                                                    <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium shadow-sm transition duration-150 ease-in-out sm:text-sm sm:leading-5 focus:border-purple-300 focus:ring focus:ring-purple-200 focus:ring-opacity-50 bg-zinc-700 text-white border border-zinc-700 hover:bg-zinc-600">{{ $page }}</span>
                                                </span>
                                            @else
                                                <button wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium shadow-sm transition duration-150 ease-in-out sm:text-sm sm:leading-5 focus:border-purple-300 focus:ring focus:ring-purple-200 focus:ring-opacity-50 bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-600" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                                    {{ $page }}
                                                </button>
                                            @endif
                                        </span>
                        ...
                        <span>
                            ...
                            @else
                                <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                    <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium shadow-sm transition duration-150 ease-in-out sm:text-sm sm:leading-5 focus:border-purple-300 focus:ring focus:ring-purple-200 focus:ring-opacity-50 bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-600 rounded-tr-md rounded-br-md" aria-hidden="true">
                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                        </svg>
                                    </span>
